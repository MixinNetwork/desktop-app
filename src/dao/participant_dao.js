...
class ParticipantDao {
  ...
  getParticipantsByConversationId(conversationId) {
    return db
      .prepare(
        'SELECT u.user_id, u.identity_number, u.full_name, u.avatar_url, u.relationship FROM participants p, users u WHERE p.conversation_id = ? AND p.user_id = u.user_id ORDER BY p.created_at'
      )
      .all(conversationId)
  }
